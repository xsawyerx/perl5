...
is no size limit to BER encoding.
...
Although perl will use multiprecision arithmetic when decoding these
values, the result is a string of digits.  Using it as a number in
operations in Perl space may well lead to a loss of precision.
